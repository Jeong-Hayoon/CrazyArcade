...
namespace hy
{
	Second::Second()
	{
		...
		Texture* Second_ = Resources::Load<Texture>(L"SecoundTimer"
			, L"..\\Resources\\Image\\UI\\Timer\\Second.bmp");

		timeranimator->CreateAnimation(L"SecoundTimer", Second_, Vector2(0.0f, 0.0f), Vector2(22.0f, 26.0f), 10, Vector2(0.0f, 0.0f), 1.f);
		timeranimator->SetScale(Vector2(0.7f, 0.9f));

		timeranimator->PlayAnimation(L"SecoundTimer", true);
	}
	Second::~Second()
	{
	}
